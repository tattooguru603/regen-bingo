...
const checkBingoWin = (bingoCard, condition) => {
    ...
    if (condition === "CROSS") {
        // Must complete middle row AND middle column
...
const updateBingoCards = (matchingNFTs, condition, bingoFilePath) => {
    ...
    bingoCards.forEach(card => {
        ...
        if (card.bingo) {
            Object.entries(card.bingo).forEach(([key, value]) => {
                let valueStr = value.toString(); // Ensure value is a string
...
module.exports = { getRandomTrait, findMatchingNFTs, updateBingoCards };
